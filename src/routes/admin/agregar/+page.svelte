<script lang="ts">
  ...
  async function agregarProducto() {
    ...
    success = false;
    try {
      const { error: insertError } = await supabase.from('productos').insert([{ nombre, descripcion, precio, imagen }]);
...
<section class="min-h-screen bg-gradient-to-br from-pink-100 via-white to-white dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 text-gray-900 dark:text-white px-4 py-20 transition-colors duration-500 font-[Inter] flex items-center justify-center relative overflow-hidden">
  ...
  <form on:submit|preventDefault={agregarProducto} class="w-full max-w-xl bg-white/60 dark:bg-gray-800/50 backdrop-blur-2xl shadow-2xl rounded-3xl p-10 space-y-6 animate-fade-in z-10">
    ...
    <input bind:value={imagen} required placeholder="URL de imagen" type="url" class="w-full px-5 py-3 rounded-2xl border border-pink-200 dark:border-gray-700 bg-white/80 dark:bg-gray-900/50 backdrop-blur-md text-gray-800 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-400 transition-all duration-300" />
