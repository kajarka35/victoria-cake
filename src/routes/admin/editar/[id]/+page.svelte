<script lang="ts">
	...
	export let data;
	let { id, nombre, descripcion, precio, imagen, imagen_path } = data.producto;

	let loading = false;
	let success = false;
	let error = '';
	let nuevaImagenFile: File | null = null;
	let vistaPrevia: string = imagen;
	...
	async function guardarCambios() {
		...
		try {
			let nuevaPath = imagen_path;
			let imagenFinal = imagen;

			if (nuevaImagenFile) {
				// 1. Eliminar la imagen anterior si existe path
				if (imagen_path) {
					await supabase.storage.from('imagenes-productos').remove([imagen_path]);
				}

				// 2. Subir la nueva imagen
				nuevaPath = `productos/${crypto.randomUUID()}-${nuevaImagenFile.name}`;
				const { error: uploadError } = await supabase.storage
					.from('imagenes-productos')
					.upload(nuevaPath, nuevaImagenFile);
				if (uploadError) throw uploadError;

				const { data: publicUrlData } = supabase.storage
					.from('imagenes-productos')
					.getPublicUrl(nuevaPath);
	...
	function handleImageChange(e: Event) {
		...
		if (target?.files && target.files.length > 0) {
			nuevaImagenFile = target.files[0];
			vistaPrevia = URL.createObjectURL(nuevaImagenFile);
		}
	}
</script>
...
>
	...
	<form
		...
		class="animate-fade-in z-10 w-full max-w-xl space-y-6 rounded-3xl bg-white/60 p-6 shadow-2xl backdrop-blur-2xl sm:p-10 dark:bg-gray-800/50"
	>
		<h1
			class="text-center text-3xl font-extrabold text-pink-600 drop-shadow-md sm:text-4xl dark:text-pink-400"
		>
			Editar Producto
		</h1>
